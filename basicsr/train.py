...
def parse_options(is_train=True):
    ...
    # a bool argument --val
    parser.add_argument(
        '--val', action='store_true', help='Run offline validation.')
    parser.add_argument(
        '--launcher',
...
def main():
    ...
    import os
    try:
        if opt['path'].get('check_resume', False):
    ...
    if resume_state is None and not args.val:
        make_exp_dirs(opt)
    ...
    epoch = start_epoch
    while not args.val and current_iter <= total_iters:
        ...
        while train_data is not None:
            ...
            # validation
            # if opt.get('val') is not None and (current_iter % opt['val']['val_freq'] == 0 or current_iter in [20, 500, 1000]):
            if opt.get('val') is not None and (current_iter % opt['val']['val_freq'] == 0 or current_iter in [150]):
    ...
    if opt.get('val') is not None:
        rgb2bgr = opt['val'].get('rgb2bgr', True)
        use_image = opt['val'].get('use_image', True)
        metric = model.validation(val_loader, current_iter if not args.val else -1, tb_logger,
                         opt['val']['save_img'], rgb2bgr, use_image)
        # if tb_logger:
